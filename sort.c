  ...
  idx_t const end)
{
  ...
  for(size_t i=start+1; i < end; ++i) {
    ...
    /* shift all data */
	//把xxxx (j)(j+1)xx (i)xxx-->xxxx (i) (j)(j+1)xx xxx
  ...
  idx_t const end)
{
  ...
  if((end-start) <= MIN_QUICKSORT_SIZE) {
    ...
    while(i < j) {
      ...
      if(p_ttqcmp2(ind0,ind1,i,imid) == 1) {//前面大于中间
        /* if tt[j] <= mid  -> swap tt[i] and tt[j]  后面小于中间*/
...
static void p_quicksort_bottom(
    ...
    idx_t bucket_size)
{
    idx_t * pos = splatt_malloc((tt->nnz + 1)* sizeof(*pos));//桶的首位置
  ...
  #pragma omp parallel
  {
    ...
    // Save the buckets
    idx_t curr = jbegin;//确保每个线程的桶号相互独立
    for(idx_t j = jbegin; j < jend; j ++){
        if (j == 0){
            pos[0] = 0;
            curr++;
    ...
    totals[tid] = curr - jbegin;//每个线程内桶的数目
    ...
    memcpy(&pos2[prefix], &pos[jbegin], sizeof(*pos2)*(totals[tid]));//将pos中可能不是顺序的桶号整合成顺序，pos2中放桶的首地址，大小只需要桶个数
  ...
  for(idx_t i = 0; i < splatt_omp_get_max_threads(); i ++){
    ...
    for(int i = 0; i < bucket_size; i ++){
        idx_t saved = cmplt[0];
        memmove(cmplt, cmplt+1, (tt->nmodes - 1) * sizeof(*cmplt));//将桶暂时移除，开始进行后面的快速排序
        cmplt[tt->nmodes-1] = saved;//桶的维度放后面，因为桶内的桶号一致，不影响比较
    }
	//guided调度是一种采用指导性的启发式自调度方法。
    ...
    idx_t m)
{
  ...
  val_t * new_vals = splatt_malloc(tt->nnz * sizeof(*new_vals));
  idx_t * histogram_array = splatt_malloc(
      (nslices * splatt_omp_get_max_threads() + 1) * sizeof(*histogram_array));//确保如果每条进程都是唯一顺序的，能够存储完
  ...
  #pragma omp parallel
  {
    ...
    /* prefix sum */
	//将每个进程的单个数字上的大小进行累加，最后再存储着一个代表着所有的计数上的总和，和累加的末尾地址，每个线程负责各自的计数大小
    for(idx_t j = (tid*nslices) + 1; j < (tid+1) * nslices; ++j) {
      idx_t transpose_j = p_transpose_idx(j, nthreads, nslices);
      idx_t transpose_j_minus_1 = p_transpose_idx(j - 1, nthreads, nslices);
    ...
    )
{
  ...
  #pragma omp parallel
  {
    ...
    jbegin = start;
    jend = end;
	//用来排序线程间可能有连续相同的桶，并在对它们排序,前面的桶只需要计数排序前面的，无需和后面比较
    if(start >= 0){
        /* count */
        for(idx_t j = jbegin; j < jend; ++j) {
            idx_t idx = tt->ind[m][j];
...
void tt_sort_range_ksadilla(//tt是对象,nmodes是张量维度值，dim_perm是排序后的顺序，k是线程数，mode是输入的排序首维度值
    sptensor_t * const tt,
    idx_t const mode,
    idx_t * dim_perm,
    idx_t const start,
