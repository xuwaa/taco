...
int transpose_coo_021_k3(struct coo_t *C_coords, int c_size, int order, int *dimensions) {
	...
	for (int i = 1; i < c_size; i++)
	{
		if (C_coords[i].idx[0] != C_coords[i - 1].idx[0]) {
			current_20++;
	...
	int B2_size = dimensions[2]+1;
	int32_t *B2_count = (int32_t *)calloc(B2_size, sizeof(int32_t));
	for (int i = 0; i < c_size; i++)
	{
		int32_t idx2 = C_coords[i].idx[2];
		B2_count[idx2]++;
	}
...
int main(int argc, char* argv[]) {
	std::string filename = "/home/xwb/transpose/taco/apps/tensor_3_fusion/nell-1.tns";
//        std::string filename = "/home/xwb/taco/transpose/taco/research/benchmark/nell-2.tns";
	...
	output.open("E:/桌面/tensor3.tns", std::ios::out);

	for (int i = 0; i < size; ++i) {
		output << A[i].idx[0] << " " << A[i].idx[1] << " " << A[i].idx[2] << " " <<A[i].val << std::endl;
	}
